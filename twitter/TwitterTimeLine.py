...
timeline = apiTwitter.GetUserTimeline(screen_name='PyConES')

print("\nStatus:",timeline[0])


print("\nFriends:")
accountFriends = [account.name for account in apiTwitter.GetFriends(screen_name='PyConES')]
    
for friend in accountFriends:
    print(friend.encode('ascii', 'ignore').decode('utf-8'))

print("\nTweets: ")
query = apiTwitter.GetSearch("#PyConES", count=100)
